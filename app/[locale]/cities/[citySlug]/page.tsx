...
export function generateStaticParams() { return CITY_REGISTRY.map((city) => ({ citySlug: city.slug })); }
...
export default function CityPage({ params }: { params: { locale: string; citySlug: string } }) {
  ...
  return <div className="pb-10">
    <section className="relative isolate flex min-h-[76svh] items-end overflow-hidden bg-[#08111f] pb-14 pt-36 text-white">
      {media?.hero ? <ThemeVideoBackground darkSrc={media.hero.src} fallbackSrc={media.gallery[0]?.src ?? media.fallback} label={localizedMediaText(media.hero.ariaLabel, locale)} className="z-0" videoClassName="object-cover object-center" /> : media?.gallery[0] ? <Image src={media.gallery[0].src} alt={localizedMediaText(media.gallery[0].alt, locale)} fill priority sizes="100vw" className="z-0 object-cover" /> : null}
      ...
      <PageContainer className="relative z-20">
        <Link href={`/${locale}/cities`} className="mb-8 inline-flex items-center gap-2 text-sm text-white/75 hover:text-white"><Arrow className="h-4 w-4 rotate-180 rtl:rotate-180" />{labels.back}</Link>
        <div className="max-w-3xl"><div className="flex flex-wrap items-center gap-3"><p className="text-sm font-semibold uppercase tracking-[.14em] text-blue-200">{region}</p><CityStatusBadge status={city.estimation.publicEnabled ? 'available' : 'coming-soon'} className="border-white/20 bg-white/10 text-white">{city.estimation.publicEnabled ? dict.phase3.cities.available : dict.phase3.cities.comingSoon}</CityStatusBadge></div><h1 className="mt-4 text-display text-white">{name}</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-white/80">{content?.subtitle ?? labels.genericIntro}</p>
          {isCasablanca ? <Link href={`/${locale}/cities/casablanca/estimate`} className="mt-8 inline-flex items-center gap-2 rounded-control bg-white px-6 py-3.5 font-semibold text-[#101b2d] transition hover:bg-blue-50">{content?.estimationCta ?? labels.estimate}<Arrow className="h-4 w-4" /></Link> : null}
        </div>
      </PageContainer>
    </section>
    ...
    {isCasablanca ? <Section><PageContainer><div className="rounded-media bg-[#101b2d] p-8 text-white sm:p-12"><h2 className="text-heading text-white">{content?.availabilityMessage ?? labels.estimate}</h2><p className="mt-4 max-w-2xl text-white/70">{dict.phase3.estimate.disclaimer}</p><Link href={`/${locale}/cities/${city.slug}/estimate`} className="mt-8 inline-flex items-center gap-2 rounded-control bg-white px-6 py-3 font-semibold text-[#101b2d]">{content?.estimationCta ?? labels.estimate}<Arrow className="h-4 w-4" /></Link></div></PageContainer></Section> : null}
  </div>;
}
