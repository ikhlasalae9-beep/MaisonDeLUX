...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!event.event_key || !event.city || !event.model_version || !event.input_features ||
        typeof event.input_features !== 'object' || Array.isArray(event.input_features) || !Number.isFinite(event.estimated_price_mad)) {
