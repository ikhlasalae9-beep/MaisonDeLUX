...
export function CasablancaSpotlight({ locale, copy }: { locale: string; copy: any }) {
  ...
  return <div className="space-y-8">
    <article className="grid overflow-hidden rounded-[2rem] border border-border-subtle bg-surface shadow-card lg:grid-cols-[1.08fr_.92fr]">
      ...
      <div className="flex flex-col justify-center p-7 sm:p-10 lg:p-12">
        ...
        <h2 className="mt-6 text-4xl font-bold text-text-primary sm:text-5xl">{locale === 'ar' ? 'الدار البيضاء' : 'Casablanca'}</h2>
        <p className="mt-5 text-base leading-8 text-text-secondary">{locale === 'ar' ? 'اكتشفوا الحاضرة الأطلسية وعمارتها وأحياءها، وافهموا بدقة نطاق التقييم الإحصائي المتاح.' : 'Découvrez la métropole atlantique, son architecture et ses quartiers, puis comprenez précisément le périmètre de l’estimation statistique disponible.'}</p>
    ...
    <div className="rounded-card border border-dashed border-border-medium bg-surface-subtle px-6 py-5 text-center text-sm text-text-secondary">{futureMessage}</div>
  </div>;
}
