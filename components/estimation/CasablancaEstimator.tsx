...
export function CasablancaEstimator({ locale, copy }: { locale: string; copy: any }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[1.45fr_.85fr] lg:items-start">
    <CardSurface className="p-5 sm:p-8"><form onSubmit={submit} noValidate>
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        {fields.map((field) => <label key={field.key}><span className="mb-2 block text-sm font-semibold">{field.label}</span><input type="number" inputMode="decimal" min={field.min} step={field.step} value={form[field.key]} onChange={(event) => update(field.key, event.target.value)} className="h-12 w-full rounded-control border border-border-medium bg-background px-4 outline-none focus:border-brand-blue focus:shadow-focus" required /></label>)}
      ...
      <Button type="submit" size="lg" loading={loading} className="mt-6 w-full sm:w-auto">{loading ? copy.loading : copy.submit}</Button>
    </form></CardSurface>
    <div className="lg:sticky lg:top-28">{result ? <CardSurface className="overflow-hidden"><div className="bg-[#101b2d] p-7 text-white"><CheckCircle2 className="h-6 w-6 text-emerald-400" /><p className="mt-5 text-sm text-white/60">{copy.resultTitle}</p><p className="mt-2 text-3xl font-bold sm:text-4xl">{new Intl.NumberFormat(locale === 'ar' ? 'ar-MA' : 'fr-MA').format(result.estimated_price_mad)} <span className="text-base font-medium text-white/60">MAD</span></p><p className="mt-4 text-xs text-white/55">{copy.modelLabel} · {result.model_version}</p></div><div className="p-6"><p className="text-sm leading-6 text-text-secondary">{copy.disclaimer}</p><button type="button" onClick={() => setResult(null)} className="mt-5 text-sm font-semibold text-brand-blue">{copy.newEstimate}</button></div></CardSurface> : <StatePanel title={copy.resultTitle} description={copy.disclaimer} />}</div>
  </div>;
}
